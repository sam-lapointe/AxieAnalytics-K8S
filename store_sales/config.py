...
class Config:
    ...
    @staticmethod
    def get_rabbitmq_queue_sales_name() -> str:
        queue_name = os.getenv("RABBITMQ_QUEUE_SALES_NAME")
        if not queue_name:
            logging.critical("RABBITMQ_QUEUE_SALES_NAME is not set.")
            raise ValueError(
                "RABBITMQ_QUEUE_SALES_NAME environment variable is required."
            )
        return queue_name
